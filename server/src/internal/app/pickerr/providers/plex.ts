...
import { PlexApi, PlexDeepLinkOptions } from '../../plex/api';
import { FieldType } from '../../plex/types/library_items';
import { filterToQueryString } from '../../plex/util';
import { MovieMatchProvider } from './types';
        ...
        if (availableTypes.includes(type.type as LibraryType) && type.Filter) {
          for (const filter of type.Filter) {
            if (filters.has(filter.filter)) {
              ...
              const filterType =
                type.Field!.find((_) => _.key === filter.filter)?.type ??
                filter.filterType;
        ...
        if (libraryItems.size) {
          for (const libraryItem of libraryItems.Metadata) {
            ...
            if (libraryItem.thumb) {
              const [, , , metadataId, , thumbId] =
                libraryItem.thumb.split('/');
