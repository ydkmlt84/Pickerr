...
import { requestNet } from '../pickerr/util/permission.js';
import { Capabilities } from './types/capabilities.js';
import { PlexMediaContainer } from './types/common.js';
import { Identity } from './types/identity.js';
import { Libraries, Library } from './types/libraries_list.js';
import type {
  FilterValue,
  FilterValues,
} from './types/library_filter_values.js';
import { LibraryItems, Meta } from './types/library_items.js';
...
export class PlexApi {
  ...
  async getServerOwner(): Promise<string> {
    const { myPlex, myPlexSigninState, myPlexUsername } =
  ...
  async getFilterValues(key: string): Promise<FilterValues> {
    if (key.includes('/')) {
      return this.fetch<FilterValues>(`/library/sections/${key}`);
    } else {
      const libraries = await this.getLibraries();
  ...
  getLibraryItems(
    ...
    return this.fetch<LibraryItems>(`/library/sections/${key}/all`, {
